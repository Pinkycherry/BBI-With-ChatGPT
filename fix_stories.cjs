...
content = content.replace(
  /How \$\{study.founder_profile.founder_alias\} scaled a \$\{study.story_theme\} to \$\$\{study.revenue.monthly_revenue_usd.toLocaleString\(\)\}\/mo\./g,
  "How this founder scaled a ${study.story_theme} without venture capital.",
);

// Financial stats card
content = content.replace(
  /\{formatUsd\(study\.revenue\.monthly_revenue_usd\)\}/g,
  '\"Premium Pricing\"',
);
content = content.replace(
  /\{formatUsd\(study\.revenue\.annual_run_rate_usd\)\}/g,
  '\"Sustainable Scale\"',
);
content = content.replace(/\{study\.revenue\.net_profit_margin_percent\}%/g, '\"High Margin\"');
content = content.replace(
  /\{formatUsd\(study\.initial_investment\.total_amount_usd\)\}/g,
  '\"Bootstrapped\"',
);
...
// Milestone revenue
content = content.replace(
  /\{formatUsd\(milestone\.revenue_reached_usd\)\}\/mo/g,
  "Milestone Achieved",
);
