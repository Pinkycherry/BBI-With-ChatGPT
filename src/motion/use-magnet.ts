...
export function useMagnet<T extends HTMLElement = HTMLElement>(
  ...
  useEffect(() => {
    ...
    return observeMotionPreference(
      () => {
        // Independent translation composes with the button's authored transform,
        // including CSS hover/press states. Restore it when the magnet settles.
        const restore = preserveStyles([el], ["translate"]);
