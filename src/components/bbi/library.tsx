...
import { BLUEPRINT_PLACEHOLDERS, contentList } from "./blueprint";
...
export function IdeaPlaceholder() {
  return <article className="library-idea-placeholder glass-panel"><div className="library-placeholder-art" aria-hidden="true"><div /><BookOpen size={42} /><div /></div><div><span className="eyebrow">Blueprint template</span><h3>{BLUEPRINT_PLACEHOLDERS.title}</h3><p className="muted">{BLUEPRINT_PLACEHOLDERS.summary}</p><div className="library-placeholder-answers"><span>Who pays</span><span>The money</span><span>Year one</span><span>Your fit</span></div></div></article>;
}

export function LibraryIdeaCard({ idea }: { idea: LibraryIdea }) {
  return <article className="glass-panel library-idea-card"><div className="library-inline"><span className="pill">{idea.category_name}</span><BookOpen size={18} aria-hidden="true" /></div><h3><Link to="/idea/$slug" params={{ slug: idea.slug }}>{idea.title}</Link></h3><p className="muted">{idea.summary}</p><dl><div><dt>Startup cost</dt><dd>{idea.startup_cost}</dd></div><div><dt>First customer</dt><dd>{idea.time_to_first_customer}</dd></div></dl><p className="library-card-verdict">{idea.verdict}</p><Link className="text-link" to="/idea/$slug" params={{ slug: idea.slug }}>Read the blueprint<ArrowRight size={17} aria-hidden="true" /></Link></article>;
}
...
export function CategoryPage({ category, ideas = NO_IDEAS, subcategory_slug, subcategory_name }: CategoryPageProps) {
  ...
  // PLACEHOLDER: a real subcategory name is supplied as a prop; route identities are never rewritten into invented names.
  const heading = subcategory_slug ? (subcategory_name ?? "Subcategory research placeholder") : category.category_name;
  return <div className="bbi-page library-page">
    ...
    <nav className="library-subcategory-rail" aria-label="Subcategories"><Link to="/category/$categorySlug" params={{ categorySlug: category.category_slug }} className={`pill${!subcategory_slug ? " library-pill-active" : ""}`} aria-current={!subcategory_slug ? "page" : undefined}>All ideas</Link>{subcategories.map((subcategory) => <Link key={subcategory.slug} to="/category/$categorySlug/$subcategorySlug" params={{ categorySlug: category.category_slug, subcategorySlug: subcategory.slug }} className={`pill${subcategory_slug === subcategory.slug ? " library-pill-active" : ""}`} aria-current={subcategory_slug === subcategory.slug ? "page" : undefined}>{subcategory.name}</Link>)}{subcategories.length === 0 && <span className="library-rail-placeholder">Subcategory placeholder</span>}</nav>

    <section aria-labelledby="category-ideas-heading"><div className="library-section-top"><h2 id="category-ideas-heading" className="section-heading">Explore the blueprints</h2><SlidersHorizontal size={20} aria-hidden="true" /></div><div className="library-filters glass-panel"><label className="library-query" htmlFor={`${id}-query`}><span className="field-label">Search within this category</span><div className="library-input-wrap"><Search size={18} aria-hidden="true" /><input id={`${id}-query`} type="search" className="input" placeholder="A skill, customer, or interest…" value={query} onChange={(event) => setQuery(event.target.value)} /></div></label><label htmlFor={`${id}-cost`}><span className="field-label">Startup cost</span><select id={`${id}-cost`} className="input" value={cost} onChange={(event) => setCost(event.target.value)}><option value="">All setup costs</option>{costs.map((item) => <option key={item} value={item}>{item}</option>)}</select></label><label htmlFor={`${id}-sort`}><span className="field-label">Sort by</span><select id={`${id}-sort`} className="input" value={sort} onChange={(event) => setSort(event.target.value)}><option value="library">Library order</option><option value="title">Title, A–Z</option></select></label></div>
