...
data.forEach((study) => {
  ...
  study.is_composite =
    "This is an illustrative composite case study representing common patterns in this business model. It does not represent a specific real founder.";
  ...
  if (study.tech_stack_and_tools) {
    study.tech_stack_and_tools = study.tech_stack_and_tools.map((tool) => {
      ...
      if (lower.includes("slack") || lower.includes("discord")) return "Team Chat";
  ...
  if (study.key_takeaways_for_founders) {
    study.key_takeaways_for_founders = study.key_takeaways_for_founders.map((t) => {
      return t
        ...
        .replace(
          /enjoy \$[0-9]+-\$[0-9]+ CPMs, 4x higher than/gi,
          "generate significantly higher advertiser returns than",
        )
  ...
  if (study.execution_milestones) {
    study.execution_milestones.forEach((m) => {
      m.key_action = m.key_action
        ...
        .replace(/Upwork/g, "freelance platforms")
        .replace(/LinkedIn/g, "professional networks");
    });
  }
});
