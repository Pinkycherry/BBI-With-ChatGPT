...
data.forEach(study => {
  ...
  if (study.tech_stack_and_tools) {
    study.tech_stack_and_tools = study.tech_stack_and_tools.map(tool => {
      ...
      if (lower.includes('claude') || lower.includes('chatgpt') || lower.includes('openai') || lower.includes('gpt-4')) return 'AI Language Model';
      ...
      if (lower.includes('google workspace') || lower.includes('g-suite') || lower.includes('google')) return 'Email/Doc Provider';
